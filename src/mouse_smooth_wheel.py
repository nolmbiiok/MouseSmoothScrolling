    ...
    if n_code == HC_ACTION and w_param == WM_MOUSEWHEEL:
        ...
        if delta != 0:
            ...
            with state_lock:
                ...
                state.velocity += delta * config.gain
    ...
    while state.running:
        ...
        with state_lock:
            if (
                ...
                and abs(state.accumulator) < config.min_send_delta
            ):
                state.velocity = 0.0
                state.accumulator = 0.0
            else:
                ...
                if abs(state.accumulator) >= config.min_send_delta:
                    send_delta = int(
                        clamp(
                            state.accumulator,
        ...
        if send_delta != 0:
            ...
            if target_root_hwnd and current_root_hwnd != target_root_hwnd:
                with state_lock:
                    cancel_inertia()
                continue
